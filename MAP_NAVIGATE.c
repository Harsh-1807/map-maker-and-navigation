...
struct Graph *createGraph(int V)
{
    ...
    for (int i = 0; i < V; i++)
    {
        ...
        graph->minDistance[i] = 100000; // Initialize minimum distances to "infinity"
    }
    return graph;
}
...
void printShortestPath(struct Graph *graph, int parent[], int dest)
{
    ...
    struct Node *edge = graph->adjList[parent[dest]];
    while (edge->data != dest)
    {
        edge = edge->next;
    }
...
Enter the filename to load the map: test1
...
Enter your choice: 1
Enter the source and destination nodes to find paths: 1
5
Paths from node 1 to node 5:
Path: 1 (left) -> 2 (left) -> 3 (left) -> 4 (right) -> 5
Total Distance: 30
Path: 1 (left) -> 2 (left) -> 3 (left) -> 5
Total Distance: 41
Path: 1 (left) -> 2 (left) -> 3 (left) -> 4 (right) -> 5
Total Distance: 28
Path: 1 (left) -> 2 (left) -> 3 (left) -> 5
...
4. Find shortest distance
Enter your choice: 4
Enter the source and destination : 1
5
Shortest path from  1 to 5 : 1 (left) -> 5  (Distance: 2)  */
